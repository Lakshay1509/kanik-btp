...
trace = {}  # per-question debate stats, reset by main.py

class Agent:
    # All model calls go through llm.chat (models.json); upstream hard-coded OpenAI/Gemini clients here.
    ...
    def chat(self, message, img_path=None, chat_mode=True):
        ...
        response = llm.chat(self.model_info, self.messages, role=self.role)
        self.messages.append({"role": "assistant", "content": response})
        return response

    def temp_responses(self, message, img_path=None):
        self.messages.append({"role": "user", "content": message})
        return {0.0: llm.chat(self.model_info, self.messages, temperature=0.0, role=self.role)}
...
def create_question(sample, dataset):
    if 'options' in sample:  # every dataset from prepare_data.py uses the MedQA schema
...
def determine_difficulty(question, difficulty, model='gpt-3.5-turbo'):
...
def process_intermediate_query(question, examplers, model, args):
    ...
    tmp_agent = Agent(instruction=recruit_prompt, role='recruiter', model_info=args.aux_model)
    ...
    for n in range(1, num_rounds+1):
        ...
        for turn_num in range(num_turns):
            ...
            trace.setdefault('num_yes', []).append([n, turn_num + 1, num_yes])
            if num_yes == 0:
                break
    ...
    trace['finals_collected'] = final_answer is not None  # upstream bug: None when a turn had no speaker
